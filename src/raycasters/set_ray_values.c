...
/*   Updated: 2025/10/13 13:57:50 by tmalkawi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_ray_values(t_game *game, int x)
{
	game->ray->camera_x = 2 * x
		/ (float)game->play->plane_x * game->ray->camera_x;
...
static void	calc_step_and_sidedist(t_game *game)
{
	...
	{
		game->ray->step_x = 1;
		game->ray->side_dist_x = (game->ray->map_x + 1.0f - game->play->pos_x)
			* game->ray->delta_dist_x;
	}
	if (game->ray->dir_y < 0)
	{
	...
	{
		game->ray->step_y = 1;
		game->ray->side_dist_y = (game->ray->map_y + 1.0f - game->play->pos_y)
			* game->ray->delta_dist_y;
	}
}
